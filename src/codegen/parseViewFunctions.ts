 ...
 */
function parseViewFunctionsFromSource(source: string, file: string): ViewFunction[] {
...
export function generateViewTypes(viewFns: ViewFunction[]): string {
  return viewFns
    .map((fn) => {
      const typeName = fn.name.charAt(0).toUpperCase() + fn.name.slice(1) + 'View';
...
export function viewFunctionToViewArgs(
  ...
  const functionName = `${moduleAddress}::${moduleName}::${fn.name}` as `${string}::${string}::${string}`;
  return {
    payload: {
 ...
 */
export function generate(moveDir: string, outFile: string, importPath = 'open-libra-sdk/types/clientPayloads') {
  const views = extractViewFunctions(moveDir);
  const lines: string[] = [];
  lines.push('// This file is auto-generated by parseViewFunctions.ts. Do not edit manually.');
  ...
  for (const fn of views) {
    ...
    const argsArray = fn.args.length ? `[${fn.args.map(a => `/* ${a.name}: ${a.type} */`).join(', ')}]` : '[]';
 ...
 * @param outFile Output .ts file path
 */
export function generateSugar(moveDir: string, outFile: string, importPath = 'open-libra-sdk/types/clientPayloads') {
  const views = extractViewFunctions(moveDir);
  const lines: string[] = [];
  lines.push('// This file is auto-generated by parseViewFunctions.ts. Do not edit manually.');
  lines.push(`import type { ViewArgs } from "${importPath}";`);
  ...
  for (const fn of views) {
    ...
    const tsFuncName =
      ...
      moduleName.slice(1) +
      fn.name.replace(/_([a-z])/g, (_, c) => c.toUpperCase()).replace(/^([a-z])/, (m) => m.toUpperCase());
    // Args signature
    const argList = fn.args.map((a, i) => `${a.name || 'arg'+i}: any`).join(', ');
    const argNames = fn.args.map((a, i) => a.name || 'arg'+i).join(', ');
    // JSDoc
    lines.push('/**');
    lines.push(` * Sugar for ${functionName}`);
    if (fn.args.length) {
      fn.args.forEach((a, i) => lines.push(` * @param ${a.name || 'arg'+i} ${a.type}`));
    }
